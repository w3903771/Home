...
class Ui_doorWindow(object):
    ...
    def setupUi(self, doorWindow):
        ...
        doorWindow.setStyleSheet(
            "#doorWindow{border-image: url(:/Resources/background.png);}\n""")
...
class MainWindow(QtWidgets.QMainWindow, Ui_doorWindow):  # 继承qtdesiner的MinWindow 确保window为qtwidgets 不然massagebox无法启用
    ...
    def update(self, flag, ishost):
        ...
        if flag:
            ...
            show = cv2.cvtColor(
                img2, cv2.COLOR_BGR2RGB)  # 视频色彩转换回RGB，这样才是现实的颜色
        ...
        if flag and not ishost:
            ...
            count = count + 1
            if time.time() - detecttime > 30 and count > 30:  # 停留时间超过2分钟 识别次数超过20次 记录为异常
                count = 0
                detecttime = 0
